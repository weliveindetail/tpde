...
This script is a utility to update 'tpde-llc' based test cases with new
FileCheck patterns. It can either update all of the tests in the file or
a single test function.

Modified from LLVM upstream llvm/utils/update_llc_test_checks.py.
"""
...
def main():
    ...
    parser.add_argument(
        "--llc-binary",
        default=None,
        help='The "tpde-llc" binary to use to generate the test case',
    )
    ...
    for ti in common.itertests(
        initial_args.tests, parser, script_name="test/" + script_name
    ):
        triple_in_ir = None
        ...
        for l in ti.run_lines:
            ...
            # tpde-llvm commands are like tpde_llvm | llvm-objdump | FileCheck
            commands = [cmd.strip() for cmd in l.split("|")]
            assert len(commands) >= 3
            preprocess_cmd = None
            if len(commands) > 3:
                preprocess_cmd = " | ".join(commands[:-3])
            if commands[-2] == "%objdump":
        ...
        for (
            ...
            scrubber, _ = output_type.get_run_handler(triple)
            function_re = OBJDUMP_FUNCTION_RE
