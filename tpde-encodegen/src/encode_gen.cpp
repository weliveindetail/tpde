    ...
    if (inst->isPseudo()) {
        if (inst->isKill()) {
            ...
            if (state.target->reg_id_from_mc_reg(inst->getOperand(0).getReg())
                != state.target->reg_id_from_mc_reg(
...
// TODO(ts): check preferred encodings
    ...
    for (const auto &cand : candidates) {
        ...
        std::vector<std::pair<bool, const llvm::MachineOperand *>>
            defs_allocated{};
        for (const auto &def : inst->all_defs()) {
        ...
        for (const llvm::MachineOperand &use : inst->operands()) {
            ...
            if (use.isReg() && use.isUse() && use.isImplicit()) {
                ...
                if (implicit_uses >= llvm_inst_desc.NumImplicitUses) {
                    state.fmt_line(
                        buf, 8, "// operand exceeds NumImplicitUses, ignore");
                    continue;
                }
                implicit_uses++;
            }
            ...
            if (use.isImplicit()) {
                ...
                state.fmt_line(buf,
                               ...
                               orig_reg_id,
                               if_cond);
                state.fixed_reg_conds[orig_reg_id].emplace_back(
                    std::format("!({})&&{}",
                                parent_conds,
                                if_cond.empty() ? "1" : if_cond));
            ...
            if (reg_info.ty == ValueInfo::SCRATCHREG) {
                const auto &def_reg = inst->getOperand(
                    inst->findTiedOperandIdx(use.getOperandNo()));
        ...
        for (const llvm::MachineOperand &def : inst->all_defs()) {
            ...
            if (def.isImplicit()
                && imp_defs++ >= llvm_inst_desc.NumImplicitDefs) {
                continue;
            }
            if (state.target->reg_should_be_ignored(reg)) {
                continue;
    ...
    const auto collect_liveouts =
        [block, &state, &buf](
            ...
            for (auto &reg : block->liveouts()) {
                ...
                const auto reg_id =
                    state.target->reg_id_from_mc_reg(reg.PhysReg);
        ...
        for (auto it = ++block->getIterator(); it != state.func->end(); ++it) {
            for (const auto &inst : *it) {
                for (const auto &op : inst.operands()) {
                    ...
                    const auto reg_id =
                        state.target->reg_id_from_mc_reg(op.getReg());
    ...
    for (auto bb_it = func->begin(); bb_it != func->end(); ++bb_it) {
        for (auto inst_it = bb_it->begin(); inst_it != bb_it->end();
            ...
            for (auto reg : MCID.implicit_defs()) {
                ...
                state.maybe_fixed_regs.insert(
                    state.target->reg_id_from_mc_reg(reg));
            }
        ...
        for (auto bb_it = func->begin(); bb_it != func->end(); ++bb_it) {
            if (multiple_bbs && !bb_it->pred_empty()) {
                ...
                for (auto &livein : bb_it->liveins()) {
                    ...
                    const auto reg_id =
                        state.target->reg_id_from_mc_reg(livein.PhysReg);
                    assert(state.value_map.contains(reg_id));
