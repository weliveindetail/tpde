...
struct AssemblerElfA64 : AssemblerElf<AssemblerElfA64> {
    ...
    VeneerInfo &find_nearest_veneer(u32                 jmp_off,
    ...
    while (cur_entry != ~0u) {
        ...
        case UnresolvedEntryKind::COND_BR: {
            ...
            if (diff < 1024 * 1024) {
                ...
                // veneer
                const auto cur_inst = *reinterpret_cast<u32 *>(
                    sec_text.data.data() + entry.text_off);
                const auto new_inst =
                    (cur_inst & 0xFF00'001F) | ((diff / 4) << 5);
                *reinterpret_cast<u32 *>(sec_text.data.data()
                                         + entry.text_off) = new_inst;
                ...
                // need to use venner
                VeneerInfo &v = find_nearest_veneer(
                    entry.text_off, unresolved_entry_kinds[cur_entry]);
                assert(v.insts_used + 5 <= v.max_insts);

                const auto trampoline_off =
                    v.off + (v.insts_used * sizeof(u32));
                auto *w = reinterpret_cast<u32 *>(sec_text.data.data()
                                                  + trampoline_off);
                // ADR x16, [pc + 4*4]
...
inline void AssemblerElfA64::reloc_data_abs(const SymRef target,
                                            ...
                                            const i32    addend) noexcept {
    if (read_only) {
        reloc_sec(sec_relrodata, target, R_AARCH64_ABS64, off, addend);
    } else {
        reloc_sec(sec_data, target, R_AARCH64_ABS64, off, addend);
    }
}

inline void AssemblerElfA64::reloc_data_pc32(const SymRef target,
                                             const bool   read_only,
                                             const u32    off,
                                             const i32    addend) noexcept {
    if (read_only) {
        reloc_sec(sec_relrodata, target, R_AARCH64_PREL32, off, addend);
    } else {
        reloc_sec(sec_data, target, R_AARCH64_PREL32, off, addend);
    }
}
...
inline void AssemblerElfA64::reloc_except_table_pc32(
    ...
    reloc_sec(sec_except_table, target, R_AARCH64_PREL32, off, addend);
}
...
inline void AssemblerElfA64::text_ensure_space(u32 size) noexcept {
    ...
    if (unresolved_cond_brs
        && (cur_off - last_cond_veneer_off)
