...
struct CallingConv {
    ...
    void fill_call_results(
        ...
        std::span<std::variant<
            typename CompilerA64<Adaptor, Derived, BaseTy, Config>::
                ValuePartRef,
...
template <IRAdaptor Adaptor,
          ...
          typename BaseTy,
          typename Config>
void CompilerA64<Adaptor, Derived, BaseTy, Config>::finish_func() noexcept {
...
void CompilerA64<Adaptor, Derived, BaseTy, Config>::generate_call(
    ...
    if (std::holds_alternative<Assembler::SymRef>(target)) {
        ...
        auto &ref = std::get<ValuePartRef>(target);
        assert(((1ull << AsmReg::R9)
                & (calling_conv.callee_saved_mask()
                   | calling_conv.arg_regs_mask()))
               == 0);
        if (ref.is_const) {
            this->register_file.clobbered |= (1ull << AsmReg::R9);
            const auto reg = ref.reload_into_specific(derived(), AsmReg::R9);
            ASM(BLR, reg);
        } else {
            ...
            if (ap.register_valid()) {
                ...
                this->register_file.clobbered |= (1ull << AsmReg::R9);
                ref.reload_into_specific(derived(), AsmReg::R9);
                ASM(BLR, DA_GP(9));
            }
